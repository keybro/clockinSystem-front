    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="18">
        <div style="text-align: center">
          <h3 style="margin-top: 1px">{{deptList[0].deptName}}--打卡情况</h3>
        </div>
      </el-col>
...
export default {
  ...
  methods: {
    async init(){
      ...
      this.queryParamsUserNameList.deptId = deptId
      // getUser(userId).then(resp=>{
      //   this.userInfo = resp.data;
      this.queryParamsScheduleGroup.depId = deptId
      this.deptqueryParams.deptId = deptId;
      listDept(this.deptqueryParams).then(response => {
        this.deptList = this.handleTree(response.data, "deptId");
        console.log(this.deptList)
      });
      await listSchedule_group(this.queryParamsScheduleGroup).then(response => {
        this.schedule_groupList = response.rows;
        // this.total = response.total;s
        this.loading = false;
      });
      await listUser(this.queryParamsUserNameList).then(response => {
    ...
    async getList() {
      this.loading = true;
      await listClockin(this.addDateRange(this.queryParams,this.dateRange)).then(response => {
